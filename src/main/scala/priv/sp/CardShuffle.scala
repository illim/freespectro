package priv.sp

/**
 * TODO :
 * 2 healers, 1 direct spell, manual exclusions
 */

import util.Random

object CardShuffle {
  
  def createOnePlayer(houses : List[House]) = {
    def eval = new PlayerState( (0 to 3).map(i => from(houses(i))).toList)
    
    var r = eval
    while(!(hasOneWipe(r) && hasOneManaGen(r))){
      r = eval
    }
    r
  }
  
  def create() = {
    def houseFilter(house: House)( f : Card => Boolean) = house.copy(cards = house.cards.filter(f))
    val baseHouses = List(Houses.FireHouse, Houses.WaterHouse, Houses.AirHouse, Houses.EarthHouse )
    val p1 = createOnePlayer(baseHouses)
    val p2 = createOnePlayer(baseHouses.zipWithIndex.map{ case (h, idx) =>
      val p1Cards = p1.houseCards(idx).cardStates.map(_.card)
      houseFilter(h){ c => !p1Cards.contains(c)}      
    })
    List(p1, p2)
  }

  def from(house: House) = PlayerHouse(house, randomize(house).sortBy(_.cost), Random.nextInt(3) + 3)

  private def randomize(house: House) = {
    import Random.shuffle
    val (c1, ctemp) = house.cards.partition(_.cost < 7)
    val (c2, c3) = ctemp.partition(_.cost < 11)
    shuffle(c1).take(2) ++ shuffle(c2).take(1) ++ shuffle(c3).take(1)
  }
  
  private val fwipes = List(6, 9)
  private val awipes = List(8)
  private def hasOneWipe(player : PlayerState) = {
    (player.houseCards(0).cardStates.count(cs => fwipes.contains(cs.card.cost))
    + player.houseCards(2).cardStates.count(cs => awipes.contains(cs.card.cost))) == 1
  }
  
  private def hasOneManaGen(player : PlayerState) = {
    def toi(b : Boolean) = if (b) 1 else 0
    def hasCard(house : Int, cost : Int) = toi(player.houseCards(house).cardStates.exists(cs => cs.card.cost==cost))
    (hasCard(0,3) + hasCard(1,5) + hasCard(3,5)) == 1
  }

}