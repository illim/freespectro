...
trait EarthHouse {
  ...
  val Earth : House = House("earth", List(
    Creature("ElvenHealer", Attack(2), 12, "Heals owner by 3 every turn", effects = effects(OnTurn -> focus(heal(3)))),
    ...
    Creature("elfHermit", Attack(1), 13, "Increase earth mana growth by 2", effects = effects(OnTurn -> addMana(2, 3))),
