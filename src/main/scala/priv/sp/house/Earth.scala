...
trait EarthHouse {
  ...
  val Earth : House = House("earth", List(
    ...
    Creature("elfHermit", Some(1), 13, "Increase earth mana growth by 2", effects = effects(OnTurn -> addMana(2, 3)), isFocusable = false),
    ...
    Creature("Troll", Some(6), 25, "Every turn heals himself by 4", effects = effects(OnTurn -> healCreature(4))),
    ...
    Creature("MasterHealer", Some(3), 35, "Every turn heals by 3 owner and his creatures",
      effects = effects(OnTurn -> heal(3), OnTurn -> healCreatures(3))),
