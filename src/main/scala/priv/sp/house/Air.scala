...
trait Air {
  ...
  val Air = House("air", List(
    Creature("Faerie novice", Attack(4), 11, "Increase spell damage by 1", mod = Some(new SpellMod(x => x + 1))),
    ...
    Creature("Air medic", Attack(4), 19, "When summoned heals owner by amount of earth mana(limited to 10)",
      effects = effects(Direct -> { env : Env =>
        env.focus()
        env.player.heal(math.min(env.getMana(3), 10))
      })),
    Creature("Lightning fence", Attack(0), 28, "Every turn deals 4 damage to opponent", effects = effects(OnTurn -> focus(damage(4, isAbility = true)))),
    ...
    Creature("Air Elemental", AttackSources().add(ManaAttack(2)), 44, "Air elemental deals 8 damage to opponent when summoned", effects = effects(Direct -> focus(damage(8, isAbility = true)), OnTurn -> addMana(1, 2))),
