...
trait Air {
  ...
  val Air = House("air", List(
    Creature("FaerieApprentice", Attack(4), 11, "Increase spell damage by 1", mod = Some(new SpellMod(x => x + 1))),
    ...
    Creature("FaerieSage", Attack(4), 19, "When summoned heals owner by amount of earth mana(limited to 10)",
      effects = effects(Direct -> { env : Env =>
        env.focus()
        env.player.heal(math.min(env.getMana(3), 10))
      })),
    Creature("WallOfLightning", Attack(0), 28, "Every turn deals 4 damage to opponent", effects = effects(OnTurn -> focus(damage(Damage(4, isAbility = true))))),
