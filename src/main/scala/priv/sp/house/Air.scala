...
trait Air {
  ...
  val Air = House("air", List(
    ...
    Creature("LightningFence", Attack(0), 28, "Every turn deals 4 damage to opponent", effects = effects(OnTurn -> focus(damage(4, isAbility = true)))),
    ...
    Spell("ChainLightning", "Deals 9 damage to opponent and his creatures", effects = effects(Direct -> damageCreatures(9, isSpell = true), Direct -> damage(9, isSpell = true))),
    ...
    Creature("AirElemental", AttackSources().add(ManaAttack(2)), 44, "Air elemental deals 8 damage to opponent when summoned", effects = effects(Direct -> focus(damage(8, isAbility = true)), OnTurn -> addMana(1, 2))),
