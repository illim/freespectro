...
trait Water {
  ...
  val Water = House("water", List(
    ...
    Creature("seasprite", Some(5), 22, "Every turn deals 2 damage to owner",
      effects = effects(OnTurn -> { env : Env => env.player.inflict(Damage(2)) })),
    Creature("MerfolkApostate", Some(3), 10, "Give 2 fire mana when summoned", effects = effects(Direct -> addMana(2, 0))),
    Creature("IceGolem", Some(4), 12, "Immune to spell & ability", immune = true),
    Creature("MerfolkElder", Some(3), 16, "Increase air mana growth by 1", effects = effects(OnTurn -> addMana(1, 2)), isFocusable = false),
    ...
    Creature("WaterElemental", None, 38, "Heals owner by 10 when summoned", effects = effects(Direct -> heal(10), OnTurn -> addMana(1, 1))),
