...
trait LostChurch {
  ...
  private val prisoner = Creature("Prisoner", Some(3), 7, "When dying loose 1 mana of each basic houses.", reaction = new PrisonerReaction)
  private val enragedPrisoner = Creature("EnragedPrisoner", Some(7), 45, "Immune to spell & ability when liberator alive.")
  ...
  val LostChurch = House("LostChurch", List(
    ...
    Creature("Preacher", Some(5), 19, "When in play normal cards cost 1 more mana.\nIncrease growth of special mana by 1.", effects = effects(OnTurn -> addMana(1, 4))),
    Creature("FalseProphet", Some(5), 19, "Give 2 mana to each basic house,\ntakes them back when dying.", effects = effects(Direct -> addMana(2, 0, 1, 2, 3))),
    Spell("WildJustice", "Deals (12 - attack) damage to each creature.",
          effects = effects(Direct -> wildJustice)),
    Creature("Scarecrow", Some(8), 26, "Deals 10 damage on opposite creature, when dying heal opposite creature by 5.", effects = effects(Direct -> { env : Env =>
        env.focus()
        env.otherPlayer.slots.inflictCreature(env.selected, Damage(10, isAbility = true))
      })),
    new SkinnedBeast,
    Creature("Falconer", Some(6), 28, "Each turns deals (2 * slot distance) damage to highest cost creature\n and creatures between.", effects = effects(OnTurn -> focus(falcon))),
    Creature("Liberator", Some(4), 15, "Turns prisoner into Enraged prisoner. When dying inflict 15 damage to him.", reaction = new LiberatorReaction, effects = effects(Direct -> focus(deliverPrisoner)))),
...
class SkinnedBeast extends Creature("SkinnedBeast", Some(8), 31, "Damage done to him is transformed into (10 - damage)."){
