...
class Houses extends HouseCardEffects {
  ...
  val Fire = House("fire", List(
    Creature("GoblinBerserker", Some(4), 16, "Every turn deals 2 damage to owner adjacent cards", spec = creature(OnTurn ->goblinBerserker)),
    ...
    Creature("PriestOfFire", Some(3), 13, "Every turn increase fire mana growth by 1", spec = creature(OnTurn -> addMana(1, 0)), isFocusable = false),
    new FireDrake,
    ...
    Creature("MinotaurCommander", Some(6), 20, "Increase attack of owner card by 1", boardEffect = Some(AddAttack(1))),
  ...
  val Water = House("water", List(
    Spell("Meditation", "Increase normal mana by 1", spec = spell(Direct -> addMana(1, 0, 2, 3))),
  ...
  val Air = House("air", List(
    ...
    Spell("LightningBolt", "Deals (5 + air power) damage to opponent",
    ...
    Creature("LightningCloud", Some(4), 20, "Attack all opponent creatures", multipleTarget = true),
  ...
  val Earth = House("earth", List(
    ...
    Creature("GiantSpider", Some(4), 21, "Spawn 2 forest spiders around him", spec = creature(Direct -> spider)),
